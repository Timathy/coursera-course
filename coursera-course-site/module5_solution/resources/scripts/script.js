jQuery(function () {
  var $toggleButton = jQuery('#toggleBtn');

  $toggleButton.on('blur', function (event) {
    var $collapsableNav = jQuery('#collapsable-nav');
    var wiw = window.innerWidth; // wiw = window.innerWidth
    if (wiw < 768) {
  ...
  document.addEventListener('DOMContentLoaded', function (event) {
    ...
    function buildAndShowHomeHTML(categories) {
      ...
      $ajaxUtils.sendGetRequest(homeHtmlUrl, function (homeHtml) {
        // Call chooseRandomCategory, passing the categories parameter
        var chosenCategoryShortName = chooseRandomCategory(categories).short_name;

        // Insert the random categories
    ...
    function chooseRandomCategory(categories) {
      ...
      // Return category object with randomArrayIndex
      ...
      $ajaxUtils.sendGetRequest(categoriesTitleHtml,
        function (categoriesTitleHtml) {
          ...
          $ajaxUtils.sendGetRequest(categoryHtml, function (categoryHtml) {
            // Switch CSS class to active to menu button
            switchMenuToActive();
    ...
    function insertItemPrice(html, pricePropName, priceValue) {
      // If not specified, replace with empy string
      if (!priceValue) {
    ...
    // Appeds portion name in parens if it exists
